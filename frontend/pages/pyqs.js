...
const PyqsPage = () => {
  ...
  const fetchPyqs = useCallback(() => {
    ...
    let url = `${BASE_API_URL}/pyqs?${params.toString()}`;
    fetch(url, {
      headers: { 'Authorization': `Bearer ${getToken()}` }
    })
      .then(res => res.json())
  ...
  }, [fetchPyqs, searchInitiated]);

  const refetchIfSearched = () => {
    if (searchInitiated) fetchPyqs();
  };
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const res = await fetch(`${BASE_API_URL}/pyq`, {
        method: "POST",
        headers: { 'Authorization': `Bearer ${getToken()}` },
        body: formData
      });
  ...
  const handleSaveEdit = async (e) => {
    ...
    form.pdfs.forEach(f => formData.append("pdfs", f));
    try {
      const res = await fetch(`${BASE_API_URL}/pyq/${editItem._id}`, {
        method: "PUT",
        headers: { 'Authorization': `Bearer ${getToken()}` },
        body: formData
      });
  ...
  const handleDelete = async (id) => {
    ...
    setStatus("Deleting...");
    try {
      const res = await fetch(`${BASE_API_URL}/pyq/${id}`, {
        method: "DELETE",
        headers: { 'Authorization': `Bearer ${getToken()}` }
      });
      if (res.ok) {
  ...
  return (
    <div style={{ padding: 48, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <button type="button" onClick={() => { setFilter({ class: role === 'student' && user && user.class ? user.class : '', subject: '', chapter: '', pdfType: '' }); setSearchInitiated(false); setPyqs([]); }} style={{ background: '#bbb', color: '#222', border: 'none', borderRadius: 6, padding: '8px 12px', fontWeight: 600, cursor: 'pointer' }}>Clear</button>
      </div>
      {/* Only show add PYQ section for non-students */}
      {role !== 'student' && (
        <>
          <button onClick={() => {
            setForm({ class: '', subject: '', chapter: '', pdfs: [], pdfType: '' });
          ...
          {/* Add PYQ Form - always visible below the button */}
      ...
      {(!searchInitiated) ? null : loading ? (
        ...
        <div style={{ color: "#888", fontSize: 17 }}>No PYQs found.</div>
      ) : (
      ...
      {showEdit && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 3000 }}>
          <form onSubmit={handleSaveEdit} style={{ background: '#fff', borderRadius: 16, padding: 32, minWidth: 320, boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.37)', textAlign: 'center', maxWidth: 420 }}>
            <h3 style={{ marginBottom: 18, color: '#1e3c72' }}>Edit PYQ</h3>
