...
export default function StudentSettingsPage() {
  ...
  const handlePrivacySave = async () => {
    ...
    try {
      ...
      const filteredPrivacy = Object.fromEntries(
      ...
      const res = await fetch(`${BASE_API_URL}/profile-visibility`, {
        ...
        body: JSON.stringify(filteredPrivacy)
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const features = [
    ...
    { key: "update-profile", label: "Update Profile", icon: <FaUserEdit style={{ fontSize: 36, marginBottom: 10, color: '#1e3c72' }} />, desc: "Edit your profile information", ref: updateProfileRef, action: () => window.location.href = "/student/profile" },
