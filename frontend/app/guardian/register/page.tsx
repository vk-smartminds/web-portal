...
export default function RegisterGuardian() {
  ...
  const [childOtp, setChildOtp] = useState<string[]>(["", "", "", "", "", ""]);
  const childOtpRefs = [
  ...
  const [otpBlocks, setOtpBlocks] = useState<string[]>(["", "", "", "", "", ""]);
  const otpRefs = [
    useRef<HTMLInputElement>(null),
    useRef<HTMLInputElement>(null),
    useRef<HTMLInputElement>(null),
  ...
  const handleOtpBlockChange = (idx: number, val: string) => {
    ...
    if (val && idx < 5 && otpRefs[idx + 1].current) {
      otpRefs[idx + 1].current?.focus();
    }
  };

  const handleOtpBlockKeyDown = (idx: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Backspace" && !otpBlocks[idx] && idx > 0 && otpRefs[idx - 1].current) {
      otpRefs[idx - 1].current?.focus();
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100vw', background: '#181c24', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{
        background: '#23283a',
        color: '#fff',
        borderRadius: 18,
        ...
        <div style={{ fontWeight: 700, fontSize: '2rem', color: '#fff', marginBottom: 8 }}>Guardian Registration</div>
        {/* Only show child email/OTP UI until childVerified is true */}
        {!childVerified ? (
          <form onSubmit={childOtpSent ? handleVerifyChildOtp : handleSendChildOtp} style={{ width: '100%' }}>
            <input style={{ width: '100%', padding: '12px 14px', marginBottom: 14, borderRadius: 8, border: '1.5px solid #3b4256', fontSize: '1rem', background: '#23283a', color: '#fff' }} name="childEmail" placeholder="Child Email (required)" value={childEmail} onChange={e => setChildEmail(e.target.value)} required disabled={childOtpSent} />
              ...
              <>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center', margin: '16px 0' }}>
                  {childOtp.map((v, i) => (
                    <input
                      key={i}
                      ref={childOtpRefs[i] as React.RefObject<HTMLInputElement>}
                      type="text"
                      inputMode="numeric"
                      maxLength={1}
                      value={v}
                      ...
                      style={{ width: 40, height: 40, textAlign: 'center', fontSize: 22, borderRadius: 8, border: '1.5px solid #3b4256', background: '#23283a', boxShadow: '0 1px 4px #e3e8f0' }}
                    />
                  ))}
                </div>
          ...
          <form onSubmit={handleRegister} style={{ width: '100%' }}>
            {/* --- Parent Name Field --- */}
            <input
              style={{ ...inputStyle, background: '#fff', border: '1.5px solid #3b4256', color: '#fff' }}
              name="name"
              type="text"
            ...
            />
            <input
              style={{ ...inputStyle, background: '#23283a', border: '1.5px solid #3b4256', color: '#64748b' }}
              name="child"
            ...
            {(!guardianExists || passwordChecked) && (
              <>
                ...
                <select style={{ ...inputStyle, background: '#fff', border: '1.5px solid #3b4256', color: '#fff' }} name="role" value={form.role} onChange={handleChange} required>
                  <option value="Father">Father</option>
                  <option value="Mother">Mother</option>
                  <option value="Guardian">Guardian</option>
                </select>
                <input style={{ ...inputStyle, background: '#23283a', border: '1.5px solid #3b4256', color: '#64748b' }} name="child" placeholder="Child Email (required)" value={childEmail} readOnly required />
                {!otpSent ? (
                  <>
                  ...
                  <>
                    <div style={{ display: 'flex', gap: 8, justifyContent: 'center', margin: '16px 0' }}>
                      {otpBlocks.map((v, i) => (
                        <input
                          key={i}
                          ref={otpRefs[i] as React.RefObject<HTMLInputElement>}
                          type="text"
                          inputMode="numeric"
                          maxLength={1}
                          value={v}
